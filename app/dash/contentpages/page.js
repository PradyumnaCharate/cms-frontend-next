...
function Page() {
  ...
  const handleDelete = async () => {
    // e.preventDefault();
    const endpoint = `${process.env.NEXT_PUBLIC_API_URL}/pages/${selectedData?.id}`;
    const method = "DELETE";
  ...
  return (
    <Container>
      <CustomTable
        ...
        paging={false}
        createOnClick={() => router.push("/dash/contentpages/add")}
        ...
        {data?.data?.items?.map((data, i) => (
          <tr key={i} className="odd" style={{ fontSize: "0.75rem" }}>
            ...
            <td className="text-center">
              ...
              />
              <EditButton
                onClick={() => router.push(`/dash/contentpages/${data?.slug}`)}
              />
              <DeleteButton
                onClick={() => {
                  setSelectedData(data);
      ...
      <ViewModal
        ...
        plugins={selectedData?.pluginConfigurations}
      />
    </Container>
  );
}
